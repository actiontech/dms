...
import (
	...
	"github.com/actiontech/dms/pkg/strconv"
...
func (f *FeishuClient) convertUsersResp(raw []*larkContact.UserContactInfo, users map[string]*UserContactInfo) {
	for _, user := range raw {
		id := strconv.NvlString(user.UserId)
		if id == "" {
			continue
		}
		...
		if email := strconv.NvlString(user.Email); email != "" {
			info.Email = email
			continue
		}
		if mobile := strconv.NvlString(user.Mobile); mobile != "" {
			info.Mobile = mobile
			continue
		}
	}
