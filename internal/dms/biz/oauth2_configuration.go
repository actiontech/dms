...
func (c *ClaimsInfo) DmsToken() (token string, cookieExp time.Duration, err error) {
	...
	customClaims := []jwt.CustomClaimFunc{jwt.WithUserId(c.UserId), jwt.WithExpiredTime(cookieExp), jwt.WithSub(c.Sub), jwt.WithSid(c.Sid)}
	if c.LoginSessionID != "" {
		customClaims = append(customClaims, jwt.WithLoginSessionID(c.LoginSessionID))
	}
	token, err = jwt.GenJwtToken(customClaims...)
	return
}
...
func (c *ClaimsInfo) DmsRefreshToken() (token string, cookieExp time.Duration, err error) {
	...
	customClaims := []jwt.CustomClaimFunc{jwt.WithUserId(c.UserId), jwt.WithExpiredTime(time.Duration(c.Exp-c.Iat)*time.Second), jwt.WithSub(c.Sub), jwt.WithSid(c.Sid)}
	if c.LoginSessionID != "" {
		customClaims = append(customClaims, jwt.WithLoginSessionID(c.LoginSessionID))
	}
	token, err = jwt.GenRefreshToken(customClaims...)
	return
}
